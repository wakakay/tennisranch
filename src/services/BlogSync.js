...
class BlogSync {
  ...
  async syncBlog() {
    ...
    try {
      ...
      // 清空目标表
      logger.info('清空目标表...');
      await targetConn.query('DELETE FROM tennisranch_4x.article');
      await targetConn.query('DELETE FROM tennisranch_4x.article_description');
      await targetConn.query('DELETE FROM tennisranch_4x.article_to_layout');
      await targetConn.query('DELETE FROM tennisranch_4x.article_to_store');
      logger.info('目标表清空完成');

      // 重置自增ID计数器
      await targetConn.query('ALTER TABLE tennisranch_4x.article AUTO_INCREMENT = 1');

      // 同步 blog 表到 article
      logger.info('开始同步 blog 表到 article...');
      ...
      if (blogs.length > 0) {
        ...
        await targetConn.query(`
          INSERT INTO tennisranch_4x.article (
        ...
        logger.info('article 表同步完成');
      }

      // 同步 blog_description 表到 article_description
      logger.info('开始同步 blog_description 表到 article_description...');
      ...
      if (blogDescriptions.length > 0) {
        await targetConn.query(`
          INSERT INTO tennisranch_4x.article_description (
        ...
        logger.info('article_description 表同步完成');
      }

      // 同步 blog_to_layout 表到 article_to_layout
      logger.info('开始同步 blog_to_layout 表到 article_to_layout...');
      ...
      if (blogToLayouts.length > 0) {
        await targetConn.query(`
          INSERT INTO tennisranch_4x.article_to_layout (
        ...
        logger.info('article_to_layout 表同步完成');
      }

      // 同步 blog_to_store 表到 article_to_store
      logger.info('开始同步 blog_to_store 表到 article_to_store...');
      ...
      if (blogToStores.length > 0) {
        await targetConn.query(`
          INSERT INTO tennisranch_4x.article_to_store (
            article_id, store_id
          ) VALUES ?
        ...
        logger.info('article_to_store 表同步完成');
      }
